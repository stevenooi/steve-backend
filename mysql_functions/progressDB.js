...
function generateRMSObject(welcome, thankyou, logo, default1, default2,slide1,slide2, target,tempSlides)
{
	let dataObj = {};
	 ...
	 dataObjSlidesInfoSlides = tempSlides;
...
{ 
	...
	req.on('end', function () { 
		...
		{ 
			...
			var dataSaltId = dataSplit[3];
			tempTarget.push(dataSaltId);
		...
		//con2.query("select t.*,s.salt_id as target from codmgr2.template t CROSS JOIN store s WHERE t.id=? AND s.id IN (" + dataIdList + ")", [dataReceived["templateId"]], function(err, rows) {
		con2.query("select sl.image_filename,t.* from codmgr2.template t LEFT JOIN  slide sl ON sl.template_id = t.id WHERE t.id=? AND sl.is_delete='0'", [dataReceived["templateId"]], function(err, rows) {
		...
		else
		{
		  ...
		  data = rows.map(row => {
			...
			tempSlides.push(row.image_filename);
			//rmsAPI.callRMS(rmsObj);							  
		  });
		  rmsObj = generateRMSObject(tempWelcome, tempThankyou, tempLogo, tempDefault1, tempDefault2,"","",tempTarget,tempSlides);
